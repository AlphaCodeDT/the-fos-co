    ...
    <header className="border-b border-brand-white/10 bg-brand-black">
      <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-5">
        <Link href="/" className="group flex items-center gap-3">
          <span className="inline-flex h-10 w-10 items-center justify-center rounded-full bg-brand-yellow text-sm font-bold text-brand-black">
            NS
          </span>
          <div>
            ...
            <p className="text-xs text-brand-white/60">Discover · Learn · Connect</p>
          </div>
        </Link>
  ...
  return (
    <footer className={cn('border-t border-brand-white/10 bg-brand-black', className)}>
      <div className="mx-auto flex max-w-6xl flex-col gap-4 px-4 py-10 md:flex-row md:items-center md:justify-between">
        <div>
          <p className="font-semibold text-brand-white">NSRCEL Founder Directory</p>
        ...
        <p className="text-sm text-brand-white/50">
          © {new Date().getFullYear()} Founders of Startups
        </p>
      </div>
    </footer>
  )
